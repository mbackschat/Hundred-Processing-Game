...
import java.util.Set;
  ...
  void updateCircles() {
    bodyAtPoint = getBodyAtPoint(mouseX, mouseY);
    ...
    for (int i = 0; i < numberOfCircles; i++) {
      ...
      if (bodyAtPoint == body) {
        ...
        final Set<Body> bodiesInContact = body.getBodiesInContact();
        ...
        if (physics.worldToScreen(circle.m_radius) <= MAX_RADIUS) {
          ...
          if (pumpSound.getAudioData() != null) {
            pumpSound.cue(0);
            pumpSound.play();
          }
  ...
  private void failLevel() {
    ...
    if (failedSound.getAudioData() != null) {
      failedSound.cue(0);
      failedSound.play();
    }
  }
  ...
  Body getBodyAtPoint(float x, float y) {
    ...
    org.jbox2d.collision.Shape[] shapes = physics.getWorld().query(aabb, 10);
    if (shapes == null) {
